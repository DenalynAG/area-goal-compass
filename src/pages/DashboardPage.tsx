import { mockAreas, mockSubareas, mockCollaborators, mockObjectives, mockKPIs, getAreaName, getUserName } from '@/data/mockData';
...
export default function DashboardPage() {
  const totalColabs = mockCollaborators.length;
  const activeObjectives = mockObjectives.filter(o => o.status === 'activo' || o.status === 'en_riesgo');
  const avgProgress = Math.round(activeObjectives.reduce((s, o) => s + o.progress_percent, 0) / (activeObjectives.length || 1));
  const atRisk = mockObjectives.filter(o => o.status === 'en_riesgo').length;
  const kpisAtRisk = mockKPIs.filter(k => getTrafficLight(k) === 'rojo').length;

  const metrics = [
    { label: 'Colaboradores', value: totalColabs, icon: Users, accent: 'text-accent' },
    { label: 'Áreas', value: mockAreas.length, icon: Building2, accent: 'text-primary' },
            ...
            <tbody>
              {activeObjectives.map(obj => (
                <tr key={obj.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-5 py-3 text-muted-foreground">{getAreaName(obj.scope_id) || getAreaName(mockSubareas.find(s => s.id === obj.scope_id)?.area_id || '')}</td>
                  <td className="px-5 py-3 text-muted-foreground">{getUserName(obj.owner_user_id)}</td>
            ...
            <tbody>
              {mockKPIs.map(kpi => (
                <tr key={kpi.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-5 py-3"><TrafficLightBadge light={getTrafficLight(kpi)} /></td>
                </tr>
              ))}
            </tbody>
          </table>
