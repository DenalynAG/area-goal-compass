...
export default function DashboardPage() {
  ...
  const avgProgress = Math.round(
    activeObjectives.reduce((s, o) => s + o.progress_percent, 0) / (activeObjectives.length || 1),
  );
              ...
              {activeObjectives.length === 0 ? (
                ...
                activeObjectives.map((obj) => {
                  const areaId =
                    obj.scope_type === "area"
                      ? obj.scope_id
                      : (subareas.find((s) => s.id === obj.scope_id)?.area_id ?? "");
                  return (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/50">
                ...
                <th className="text-left px-5 py-3 font-medium text-muted-foreground">Real</th>
              ...
              {kpis.length === 0 ? (
                ...
                kpis.map((kpi) => (
                  <tr key={kpi.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <TrafficLightBadge light={getTrafficLight(kpi as any)} />
                    </td>
                  </tr>
                ))
              )}
