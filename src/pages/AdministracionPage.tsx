...
const rolesModules = ['Dashboard', 'Estructura', 'Colaboradores', 'Objetivos', 'Indicadores', 'Reportes', 'Evaluaciones', 'Administración'];
const rolesActions = ['Ver', 'Crear', 'Editar', 'Eliminar'];
const rolesList = Constants.public.Enums.app_role;

const permMatrix: Record<string, Record<string, string[]>> = {
  super_admin: Object.fromEntries(rolesModules.map(m => [m, rolesActions])),
  admin_area: {
    Dashboard: ['Ver'], Estructura: ['Ver', 'Crear', 'Editar'], Colaboradores: ['Ver', 'Crear', 'Editar'],
    Objetivos: rolesActions, Indicadores: rolesActions, Reportes: ['Ver'], Evaluaciones: rolesActions, Administración: [],
  },
  lider_subarea: {
  ...
  solo_lectura: Object.fromEntries(rolesModules.map(m => [m, ['Ver']])),
};
  ...
  return (
    <div className="bg-card rounded-xl border shadow-sm">
      <div className="px-5 py-4 border-b flex items-center gap-3">
        ...
        <div>
          <h3 className="font-semibold">Roles y Accesos</h3>
          <p className="text-xs text-muted-foreground mt-0.5">Matriz de permisos por módulo</p>
        </div>
      </div>
      <div className="p-4 space-y-4">
        {rolesList.map(roleCode => {
          ...
          return (
            <div key={roleCode} className="border rounded-lg overflow-hidden">
              <div className="px-4 py-3 bg-muted/30 flex items-center gap-2">
                <Shield className="w-4 h-4 text-primary" />
                <div>
                  <span className="font-semibold text-sm">{getRoleLabel(roleCode)}</span>
                  <span className="text-xs text-muted-foreground ml-2">{roleDescriptions[roleCode] ?? ''}</span>
                </div>
              </div>
