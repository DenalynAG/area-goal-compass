        ...
        <div className="hidden lg:block w-[280px] shrink-0">
          <div className="sticky top-4">
            {(() => {
              ...
              // List of birthdays this calendar month
              const bdList = profiles
                .filter((p) => {
                  if (!p.birthday) return false;
                  return parseISO(p.birthday).getMonth() === calendarMonth.getMonth();
                })
                .sort((a, b) => parseISO(a.birthday!).getDate() - parseISO(b.birthday!).getDate());
                  ...
                  <div className="grid grid-cols-7 gap-px">
                    ...
                    {days.map((day) => {
                      ...
                      return (
                        <div
                          key={d}
                          className={`aspect-square rounded-md flex flex-col items-center justify-center relative group transition-colors ${
                            bds.length > 0
                              ? "bg-[hsl(var(--warning)/0.12)] hover:bg-[hsl(var(--warning)/0.25)] cursor-pointer"
                              : "hover:bg-muted/50"
                          ...
                          <div key={p.id} className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-full bg-primary/10 flex items-center justify-center shrink-0 overflow-hidden">
                              {p.avatar ? (
                                ...
                                <span className="text-[9px] font-bold text-primary">{getInitials(p.name)}</span>
                              )}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-xs font-medium truncate">{p.name}</p>
                              <p className="text-[10px] text-muted-foreground">
                                {format(bd, "d 'de' MMMM", { locale: es })}
                                {dayIsToday && " · 🎉 ¡Hoy!"}
                              </p>
                            </div>
                          </div>
