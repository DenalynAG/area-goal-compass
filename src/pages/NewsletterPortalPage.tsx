...
export default function NewsletterPortalPage() {
  ...
  if (isLoading) return <div className="flex items-center justify-center py-20 text-muted-foreground">Cargando portal…</div>;
    ...
    <div className="animate-fade-in space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Cumpleaños del Mes', value: birthdaysThisMonth.length, icon: Cake, accent: 'text-[hsl(var(--warning))]' },
        ].map(m => (
        ...
        <div className="bg-card rounded-xl border shadow-sm">
          ...
          <div className="p-4 space-y-3 max-h-80 overflow-y-auto">
            {birthdaysThisMonth.length === 0 ? (
              ...
              return (
                <div key={p.id} className={`flex items-center gap-3 p-3 rounded-lg transition-colors ${isToday ? 'bg-[hsl(var(--warning)/0.1)] ring-1 ring-[hsl(var(--warning)/0.3)]' : 'hover:bg-muted/50'}`}>
